...
HANDOFF_CONTRACTS = {
    ...
    "qa_to_code_review": {
        "infra": [
            "qa-engineer/templates/test_plan.md",
            "qa-engineer/templates/bug_report.md",
            "code-reviewer/templates/review_report.md",
...
def validate_handoff(base_path, handoff_type, feature_name=None):
    ...
    if feature_name and contract["feature_artifacts"]:
        for rel_path_tpl in contract["feature_artifacts"]:
            rel_path = rel_path_tpl.format(feature_name=feature_name)
            full_path = os.path.join(base_path, rel_path)
            if not os.path.exists(full_path):
                missing_files.append(rel_path)
    elif not feature_name and contract["feature_artifacts"]:
        print(
            f"Aviso: Nome da feature não fornecido. Validando apenas infraestrutura para '{handoff_type}'."
        )

    if missing_files:
        return (
            False,
            f"Falha no Handoff {handoff_type}: Arquivos obrigatórios ausentes: {', '.join(missing_files)}",
        )

    return (
        True,
        f"Sucesso: Todos os pré-requisitos para o handoff '{handoff_type}' (Feature: {feature_name or 'Global'}) foram encontrados.",
    )
